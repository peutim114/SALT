...
class ResNet(nn.Module):
    def __init__(
        ...
        self.LTformer = MY_NET(image_size=32,   patch_size=8, num_classes=2,  embed_dim=256, seq_len=0, hidden_dim=2048,
               num_heads=256,
               num_layers=4,
               dropout=0.1,
               classification=True)
    ...
    def forward(self, x, res5=False):
       ...
       # x_mid = self.layer3(x)
       # x = x.reshape(-1,)
        x_mid = self.LTformer(x)

        #print('layer4', x_mid.shape)
        x = self.LTformer(x)
       # x = self.layer4(x_mid)
       # print('layer4',x.shape)
        ...
        if not res5:
            ...
            x_mid = x_mid.reshape(-1, 17 , 16, 16)
           # print('1111111111111111111',x_mid.shape)
            feat_2 = self.trans(x_mid)
            x2 = self.avgpool(feat_2)
            x2 = torch.flatten(x2, 1)
       # x_mid = x_mid.reshape(-1, 256, 8, 8)
